...
function buildCandidateVariables(rows, intent, query) {
  ...
  for (const group of groups.values()) {
    ...
    const candidate = {
      ...
      unitFacet: bestUnit.unitFacet,
      productName: group.productName,
      activityName: group.activityName,
      unit: bestUnit.unit,
      availableUnitCount: group.units.size
    };
...
function choosePreferredUnit(group) {
  const units = Array.from(group.units.values());
  if (units.length === 0) return null;

  return units
    .map(unit => {
      const cleanRows = cleanDataRows(unit.rows);
      return {
        ...unit,
        observationCount: cleanRows.length,
        unitScore: scoreUnitPreference(unit, group)
      };
    })
    ...
    .sort((a, b) => {
      ...
      return String(a.unitFacet).localeCompare(String(b.unitFacet));
    })[0] || null;
}
...
function scoreUnitPreference(unit, group) {
  ...
  if (unitFacet === "twh" || unitName.includes("terawatthours")) score += 75;
  if (unitFacet === "gwh" || unitName.includes("gigawatthours")) score += 70;
  if (unitFacet === "bcf" || unitName.includes("billion cubic feet")) score += 65;
  if (unitFacet === "mtoe" || unitName.includes("oil equivalent")) score += 45;
  if (unitFacet === "tj" || unitName.includes("terajoules")) score += 35;

  if ((product.includes("petroleum") || product.includes("oil")) && activity.includes("consumption") && text.includes("barrels per day")) score += 30;
  if (product.includes("electricity") && (text.includes("watthours") || text.includes("watt hours"))) score += 30;
